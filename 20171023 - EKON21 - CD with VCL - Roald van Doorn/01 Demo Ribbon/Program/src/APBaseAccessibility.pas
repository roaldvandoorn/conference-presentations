...
begin
  ...
  index := varChild;
  OutputDebugString(PChar((Format('%s.Get_accChild: name=%s, interface=%s, varChild=%d, Result=%4x', [Self.ClassName, objName, intfName, index, Result]))));
...
begin
  ...
  Result := S_OK;
  OutputDebugString(PChar((Format('%s.Get_accChildCount: name=%s, childCount=%d, Result=%4x', [Self.ClassName, GetAccessibleName, pcountChildren, Result]))));
end;
...
begin
  ...
  index := varChild;
  OutputDebugString(PChar((Format('%s.Get_accName: pszName="%s", varChild=%d, Result=%4x', [Self.ClassName, pszName, index, Result]))));
end;
...
begin
  ...
  index := varChild;
  OutputDebugString(PChar((Format('%s.Get_accRole: name="%s", varChild=%d, role=%d, Result=%4x', [Self.ClassName, name, index, roleID, Result]))));
end;
...
begin
  ...
  if TVarData(varChild).VType = VT_I4 then
    OutputDebugString(PChar((Format('%s.Get_accRole: name="%s", varChild=%d, state=%d, Result=%4x', [Self.ClassName, name, TVarData(varChild).VInteger, state, Result]))))
  else
    OutputDebugString(PChar((Format('%s.Get_accRole: name="%s", varChildType=%d, state=%d, Result=%4x', [Self.ClassName, name, TVarData(varChild).VType, state, Result]))))
...
begin
  ...
  begin
    ...
    OutputDebugString(PChar((Format('%s.WMGetObject: Message.Result=%8x', [Self.ClassName, DWORD(Message.Result)]))));
  end
  else
    FOrgWndProc(Message);
...
begin
  if HasLinkObject then
  begin
    OutputDebugString(PChar((Format('%s.WMDestroyLink: WinControl=%s', [Self.ClassName, WinControl.Name]))));
    WinControl.WindowProc := FOrgWndProc;
    ClearLinkObject;
  end;
  FOrgWndProc(Message);
